...
### TaylorDub 프로젝트 직접 적용

1. **FunCineForge 오픈소스 통합**: TaylorDub 파이프라인의 립싱크 백엔드로 FunCineForge를 통합하는 PoC 설계. 특히 '시간 모달리티' 접근법이 멀티캐릭터 장면에서의 TaylorDub 성능을 크게 향상시킬 가능성.

2. **벤치마크 프레임워크 구축**: LRS3, HDTF 등 표준 데이터셋에서 FunCineForge vs Perso AI vs Synthesia의 정량 비교를 수행하여, TaylorDub 파이프라인의 품질 기준선 확보. 지난 수렴 분석에서 지적된 "립싱크 벤치마크 공백"을 해소하는 작업.

3. **오클루전 대응 파이프라인**: Perso AI의 오클루전 핸들링 기법(자동 마스킹, 하안면 트래킹)을 참조하여, TaylorDub의 실전 영상 대응력 향상.

4. **Seedance 2.0 하이브리드**: 네이티브 오디오-비디오 동시 생성(Seedance)과 기존 영상의 립싱크 변환(FunCineForge)을 조합하여, 신규 콘텐츠 생성과 기존 콘텐츠 로컬라이제이션을 모두 커버하는 하이브리드 아키텍처 설계.

### 한계 및 고려사항

- FunCineForge의 구체적 정량 벤치마크(LRS3/HDTF 스코어)는 아직 공개되지 않아 직접 실험 필요
- 130+ 언어 지원은 상용 플랫폼(Synthesia, HeyGen)의 수치이며, 오픈소스 모델의 언어 커버리지는 제한적일 수 있음
- 밀리초 단위 정밀도의 실제 체감 품질은 언어별로 차이가 클 수 있음 (한국어/일본어 등 입모양 패턴이 영어와 상이)
