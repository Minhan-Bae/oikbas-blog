...
summary: Causal multi-shot 스트리밍 아키텍처가 Memesis ReactFlow 노드 기반 인터랙티브 영상 편집에 직접 참조.
...
### Memesis 프로젝트 — 핵심 참조 (relevance 5)

- **ReactFlow 노드 기반 인터랙티브 편집**: ShotStream의 스트리밍 생성 패러다임은 Memesis의 ReactFlow 노드 기반 영상 편집 파이프라인에 직접 대응한다. 사용자가 노드를 연결하며 "다음 씬"을 지시하면 실시간으로 영상이 생성되는 워크플로의 백엔드 아키텍처로 ShotStream 패턴을 참조 가능.
- **멀티 클립 시퀀싱**: next-shot generation 패러다임은 Memesis의 멀티 클립 시퀀싱 기능 설계의 핵심 레퍼런스. 각 클립을 하나의 "샷"으로 매핑하고, 이전 클립의 컨텍스트를 조건으로 다음 클립을 생성하는 파이프라인 설계에 직접 적용.
- **듀얼 캐시 → 세션 메모리**: Global/Local Context Cache 구조는 Memesis의 편집 세션 메모리 아키텍처에 직접 참고 가능. 전역 프로젝트 스타일(Global)과 현재 편집 컨텍스트(Local)를 분리 관리하는 설계.
- **DMD 증류 전략**: 고품질 양방향 모델에서 실시간 인과 모델로의 증류 파이프라인은, Memesis가 프로덕션급 비디오 모델을 실시간 프리뷰용으로 경량화할 때 참조할 핵심 방법론.
